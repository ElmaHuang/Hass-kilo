...
class PollingHandler(asyncore.dispatcher):
    ...
    def handle_read(self):
        ...
        check_result = self.check_services(self.test)
    ...
    def check_services(self, test=False):
        ...
        if conn == None:
            message = "libvirt;"
            if test ==False:
                subprocess.check_output(['service', 'libvirt-bin', 'restart'])
            elif test==True:
        ...
        if "nova-compute" not in output:
            message += "nova;"
            if test==False:
                subprocess.check_output(['service', 'nova-compute', 'restart'])
            elif test==True:
        ...
        if "start/running" not in output:
            message += "qemukvm;"
            if test==False:
                subprocess.check_output(['service', 'qemu-kvm', 'restart'])
            elif test==True:
